...
L1 lasso penalty and L2 ridge penalty, was analyzed. The mixing parameter, 
alpha, is used in the penalty as

$$(1-\alpha)/2 ||\beta||_2^2 + \alpha||\beta||_1$$
such that $\alpha = 1$ is the lasso penalty and $\alpha = 0$ is the ridge 
penalty. The top 15 variables selected by the specific mixing parameter were 
then used to perform logistic regression.
...
ylab('Sensitivity Estimate') + ggtitle('Sensitivity Estimates Across Varying Mixing Parameters')
...
sensitivity and specificity, but the pure ridge regression $alpha$ yields lower 
...
ft <- set_caption(ft, "Sensitivity Confidence Intervals")

ft

df <- data.frame(c(base_metrics[2,1], site7_metrics[2,1]), 
                 c(base_metrics[2,2], site7_metrics[2,2]), 
                 c(base_metrics[2,3], site7_metrics[2,3]))
colnames(df) <- c("Estimate", "Lower CI Bound", "Upper CI Bound")
rownames(df) <- c("Orig. Training Data", "Site 7 Data")
df <- df %>% rownames_to_column(" ")
...
ft <- set_caption(ft, "Specificity Confidence Intervals")

ft
```
As expected, slightly worse performance is observed, as the records from site 7 
were excluded from training. In addition, the confidence interval is wider due 
to the reduced size of the test set.
...
df <- data.frame(c(base_metrics[2,1], subsample_metrics[2,1]), 
                 c(base_metrics[2,2], subsample_metrics[2,2]), 
                 c(base_metrics[2,3], subsample_metrics[2,3]))
colnames(df) <- c("Estimate", "Lower CI Bound", "Upper CI Bound")
rownames(df) <- c("Orig. Training Data", "Case and EMS Data")
df <- df %>% rownames_to_column(" ")
...
The model performs significantly worse in terms of sensitivity when the
subsetted data is applied to it, although this difference is likely due to

...
# 
# tp <- sum(avn$ControlType == "case" & avn_reprod2==1)
# tn <- sum(avn$ControlType != "case" & avn_reprod2!=1)
# sensitivity <- DescTools::BinomCI((tp), n = sum(avn$ControlType == "case"), method = "wald", conf.level = 0.95)
# specificity <- DescTools::BinomCI((tn), n = sum(avn$ControlType != "case"), method = "wald", conf.level = 0.95)
# 
# df <- rbind(sensitivity, specificity)
# colnames(df) <- c("Estimage", "Lower CI Bound", "Upper CI Bound")
# rownames(df) <- c("Sensitivity", "Specificity")

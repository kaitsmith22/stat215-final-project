...
data = read.csv("CSpine/CSV datasets/data.csv", header = TRUE)
data = subset(data, select=-c(1))
data = mutate(data, CSFractures = as.integer(CSFractures=="Y"))
data = mutate(data, TotalGCS = as.integer(TotalGCS))
data = subset(data, select=-c(TotalGCS))

# split data into validation and test/train sets
data_validation = filter(data, SITE %in% c(7,15,16))
data_eda = filter(data, !SITE %in% c(7,15,16))
...
ggplot(data_fracture_rate) +
    geom_bar( aes(x=completeness, y=mean), stat="identity", fill="skyblue", alpha=0.7) +

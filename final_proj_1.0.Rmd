---
title: "Learning and Evaluating Clinical Decision Rules for Cervical Spine Injuries"
...
plot_demographics_count <- ggplot(data = av_dem[av_dem$Race != "PI" & av_dem$Race != "AI",]) + 
                            ...
                            scale_y_discrete(labels=c("W" = "White", "O" = "Other","ND" = "Undisclosed",
                                                      "B" = "Black","A" = "Asian")) +
                            theme_bw()
...
We also used the same features developed by Leanoard et al. These features 
provide data on how the patient was injured, any pre-existing condition the patient may have,
and the symptoms of the patient, which are defined in Table 2 of Leonard et al. 
Leonard et al. determined 6 major variables that are associated with CSI: altered mental status, focal neurological deficit, complaint of neck
...
practice, the data was split into test and training sets based on sites. Sites 5, 16, and 17 were randomly chosen as the evaluation sites. Leave-one-out-cross-validation (LOOCV) was conducted over the remaining sites 
during training. 
...
```{r, fig.caption = "Plot of order features are added to the Lasso model, as the L1 Norm increases. For a large L1 Norm, the lasso solution is equivalent to the OLS solution.", fig.height=3.5, fig.width=6}
...
#  padding(padding = 1, part = "header") 
#ft
knitr::kable(sorted_sum, "pipe", digits = 3)
```
...
The list of predictors used and the decition tree itself are the following:

```{r, echo=FALSE}
knitr::kable(tree.1$frame[tree.1$frame$ncompete==4,1], "pipe", col.names = c("Used Covariates"))
```
...
```{r glmnet-alpha, cache=TRUE, echo = FALSE, include=TRUE, message = FALSE, warning= FALSE, fig.width=6, fig.align='center'}
...
ggplot(alpha_auc_df, aes(x, y)) + geom_point() + geom_line() + 
  xlab('Lasso-Ridge Mixing Parameter') + 
ylab('AUC') + ggtitle('Area under ROC Curve Across Varying Mixing Parameter') +
  theme_bw()
...
```{r Bootstrap-Lasso, fig.width=6, fig.align='center', cache=TRUE}
source("./routines.R")
library(glmnet)
library(ggplot2)
...
df_freq$Feature <- factor(df_freq$Feature, levels=(df_freq %>% arrange(Frequency))$Feature)

ggplot(df_freq, aes(y = Feature, x = Frequency)) + geom_bar(stat= "identity", fill = "palegreen3") + 
  ylab("") +
  labs(title = "Frequency of Selection By Bootstrapping") + theme_bw()
...
```{r Bootstrap-Logistic, cache=TRUE, fig.height=3, fig.width=8, message = FALSE, warning= FALSE}
require(gridExtra)
...
# Creating scatter plot with its
# confindence intervals
CI_plot1 <- ggplot(CI_bootstrap_sens, aes(x, sens)) + geom_point() + 
  geom_errorbar(aes(ymin = sens_low, ymax = sens_high))+ xlab('Bootstraps (Sorted by Sensitivity Est.)') + 
  ylab('Sensitivity Estimate') + ggtitle('Sensitivity Estimates Across Bootstraps') + theme_bw()
...
# Creating scatter plot with its
# confindence intervals
CI_plot2 <- ggplot(CI_bootstrap_spec, aes(x, spec)) + geom_point() + 
...
grid.arrange(CI_plot1, CI_plot2, ncol=2)
```
...
```{r Bootstrap-Logistic-hist, cache=TRUE, fig.height=3, fig.width=5}
...
hist(boot_auc_df$y, 
     main="Bootstrap AUC for Logistic Regression",
     xlab = "AUC")


```
To better visualize the stability across bootstraps, we plot the area under the 
ROC curve for each logistic regression classifier generated for each bootstrap. 
We see the AUC is roughly normally distributed and centered around an area of 
0.8, which seems stable enough.
...
#df <-  round(df,3)
#df <-  cbind(c("Sensitivity", "Specificity"), df)
colnames(df) <- c("Estimate", "Lower CI Bound", "Upper CI Bound")
...
knitr::kable(df, "pipe", digits = 3, caption = 'Decision Rule from Leonard et al. (2011)')
```
...
```{r sensitivity from forward selection, echo = FALSE, include=FALSE, message = FALSE, warning= FALSE, cache=TRUE}
av.sense <- av %>% drop_na(vars) %>% filter(ControlType == "case")
...
```{r, echo = FALSE, include=TRUE, message = FALSE, warning= FALSE, out.width = 6, fig.align='center'}
#kable(round(df,2), align = 'cccc',
#      caption = 'Forward Variable Selection') %>%
#  kable_styling(bootstrap_options = "striped", full_width = F, latex_options = "HOLD_position")

knitr::kable(df, "pipe", digits=3, caption = 'Forward Variable Selection')
```
...
```{r, cache=TRUE, echo = FALSE, include=TRUE, message = FALSE, warning= FALSE, out.width = '100%', fig.align='center'}
...
r <- rocs(avnn$ControlType == "case", num_preds, class_preds_tree,  avnn_reprod, avnn_reprod2)
...
knitr::kable(single_tree_metrics, "pipe", digits=3, caption = 'Single Decision Tree with 10 Variables')
```
...
```{r gradient boosted trees or stubs, cache=TRUE, echo = FALSE, include=TRUE, message = FALSE, warning= FALSE, out.width = '100%', fig.align='center'}
...
#kable_styling(bootstrap_options = "striped", full_width = F, latex_options = "HOLD_position")
knitr::kable(gb_tree_metrics, "pipe", digits=3, caption = 'Gradient-Boosted Tree                  ')
```
...
```{r logistic regression (2), echo = FALSE, include=TRUE, message = FALSE, warning= FALSE, fig.align='center'}
# CHANGE FACTORS 
...
knitr::kable(LR_metrics, "simple", digits=3, caption = 'Logistic Regression') %>% kable_styling(full_width = T)
```
...
set on data points the model miss-classified for both true positives and true negatives. Although the decision tree split the data on 4 features,
 ...
 where the "ambulatory" feature was present in the data were misclassified: both in
  ...
  tree, FocalNeuroFindings2, is present in a high percentage of the correctly labelled positive cases,
   while it is present in a smaller percentage of the incorrectly labelled negative cases. Ultimately, 

...
r <- rocs(avnn$ControlType == "case", num_preds, class_preds,  avnn_reprod, avnn_reprod2)
single_tree_metrics <- metrics(avnn$ControlType == "case", class_preds)
```
...
To interpret potential patterns in the errors, we examine the means of each feature for the misclassified groups and correctly classified groups. __TO DO: need more explanation__

```{r, echo = FALSE, include=TRUE, message = FALSE, warning= FALSE, out.width = '80%', fig.align='center'}
# TESTING DATA
data_test <- av %>% filter(SITE == 5 | SITE == 16 | SITE == 15)
y <- unlist(lapply(data_test$ControlType, function(x){
  if (x == "case")
    {return (1)} 
  else 0}))
data_test$y <- y
dat_no_na <- data_test[rowSums(is.na(data_test)) == 0, ]



fake_eval_correct <- dat_no_na[1:10,][, 5:37]

fake_eval_false <- dat_no_na[11:20,][, 5:37]

means_correct <- apply(fake_eval_correct, MARGIN = 2, function(x){
  col_num <- as.numeric(x)
  return(mean(col_num))
})

means_false <- apply(fake_eval_false, MARGIN = 2, function(x){
  col_num <- as.numeric(x)
  return(mean(col_num))
})

means_comp <- data.frame(names(means_correct), means_correct, means_false)
colnames(means_comp) <- c("Feature", "Correct", "Incorrect")

ggplot(means_comp, aes(y = Feature)) + geom_bar(stat="identity", fill = "green", aes(x = Correct)) + geom_bar(stat="identity", fill= "blue", aes(x = Incorrect)) +
  labs(title = "Average Values: Correct vs. Incorrect ")
```
